...
method grava_cliente() class cliente
    ...
    cNome       := left(self:nome           ,tamSX3('A1_NOME'   )[1])
    cNomeRed    := left(self:nome_reduzido  ,tamSX3('A1_NREDUZ' )[1])
    cEndereco   := left(self:endereco       ,tamSX3('A1_END'    )[1])
    cBairro     := left(self:bairro         ,tamSX3('A1_BAIRRO' )[1])
    cCidade     := left(self:cidade         ,tamSX3('A1_MUN'    )[1])
    cEmail      := left(self:email          ,tamSX3('A1_EMAIL'  )[1])
    cComplem    := left(self:complemento    ,tamSX3('A1_COMPLEM')[1])    

    aadd(aSA1,{"A1_COD"     ,cCodigo                ,})
    aadd(aSA1,{"A1_LOJA"    ,cLoja                  ,}) 
    aadd(aSA1,{"A1_NOME"    ,cNome                  ,})
    aadd(aSA1,{"A1_NREDUZ"  ,cNomeRed               ,}) 
    aadd(aSA1,{"A1_CGC"     ,self:cpf_cnpj          ,})
    aadd(aSA1,{"A1_TIPO"    ,"F"                    ,})
    aadd(aSA1,{"A1_PESSOA"  ,"J"                    ,})
    aadd(aSA1,{"A1_END"     ,cEndereco              ,}) 
    aadd(aSA1,{"A1_BAIRRO"  ,cBairro                ,}) 
    aadd(aSA1,{"A1_EST"     ,self:uf                ,})
    aadd(aSA1,{"A1_MUN"     ,cCidade                ,})  
    aadd(aSA1,{"A1_COD_MUN" ,self:cod_ibge          ,})   
    aadd(aSA1,{"A1_CEP"     ,self:cep               ,})
    aadd(aSA1,{"A1_EMAIL"   ,cEmail                 ,})
    aadd(aSA1,{"A1_TEL"     ,self:telefone          ,})
    aadd(aSA1,{"A1_CNAE"    ,self:cnae              ,})   
    aadd(aSA1,{"A1_COMPLEM" ,cComplem               ,})
